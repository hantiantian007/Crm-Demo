        ...
        <div class="bg-white rounded-xl card-shadow border border-gray-100 overflow-hidden">
          <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between gap-3 flex-wrap">
            <div class="flex items-center gap-3">
              ...
              <div>
                <div class="text-base font-semibold text-gray-800">支付配置记录</div>
                ...
                <table class="w-full text-sm">
                  <thead class="bg-tableHeader text-gray-600 border-b border-gray-200">
                    <tr>
                      ...
                      <th class="px-4 py-3 font-medium whitespace-nowrap">支付通道</th>
...
const logs = ref([
  ...
  {
    id: 'l2',
    time: '2026-09-15 09:12:03',
    user: 'Olivia',
    channel: 'alpay',
    payNo: 'alpay',
    opType: '禁用',
    changes: [{ key: 'status', label: '状态', before: '启用', after: '禁用' }],
    remark: ''
  }
])

const logChannelOptions = computed(() => Array.from(new Set(logs.value.map((x) => x.channel))))
...
const rateChannelOptions = computed(() => Array.from(new Set(dailyRates.value.map((x) => x.channel))))
