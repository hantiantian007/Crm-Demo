 ...
 <div class="bg-white rounded shadow-sm border border-gray-200">
  ...
  <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between gap-3">
   ...
   <div class="flex items-center gap-2">
    ...
    <button class="h-9 px-3 rounded bg-primary hover:bg-primaryHover text-white text-sm font-medium transition-colors" type="button" @click="goEdit">
     编辑
    </button>
   </div>
  </div>
  ...
  <div class="p-6 w-full">
     ...
     <div class="form-row items-center">
      ...
      <div class="form-content relative" id="salesNameContainer">
       <div class="relative w-[400px]">
        <input v-model="form.salesName" autocomplete="off" class="form-input pr-8 w-full" id="salesNameInput" placeholder="请输入销售名称或邮箱进行检索..." type="text"/>
   ...
   <div class="section-card">
    ...
    <div class="section-body">
     <div class="form-row items-center">
      ...
      <div class="form-content">
       <select v-model="form.tradeRewardEnabled" class="form-select">
        <option value="">
         请选择
        </option>
        <option value="1">
     ...
     <div id="tradeRewardConfigSection">
      <div class="form-row items-center">
       ...
       <div class="form-content flex items-center gap-6">
        <label class="flex items-center cursor-pointer">
         <input v-model="form.tradeRewardType" class="w-4 h-4 text-primary focus:ring-primary border-gray-300" name="reward_type" type="radio" value="unified"/>
         <span class="ml-2 text-sm text-gray-700">
          统一计算
         </span>
        </label>
        <label class="flex items-center cursor-pointer">
         <input v-model="form.tradeRewardType" class="w-4 h-4 text-primary focus:ring-primary border-gray-300" name="reward_type" type="radio" value="rule"/>
         <span class="ml-2 text-sm text-gray-700">
          规则计算
         </span>
        </label>
         ...
         <div class="border border-gray-200 rounded overflow-hidden shadow-sm">
          <table class="w-full text-sm text-center border-collapse bg-white">
           ...
           <tbody>
            ...
            <tr>
             <td class="p-2 border-r border-gray-200">
              <input class="w-full text-xs border border-gray-300 rounded p-1.5 text-center focus:border-primary outline-none" placeholder="请输入" type="text"/>
             </td>
             <td class="p-2 border-r border-gray-200">
              <input class="w-full text-xs border border-gray-300 rounded p-1.5 text-center focus:border-primary outline-none" placeholder="请输入" type="text"/>
             </td>
      ...
      <div class="form-row items-center">
       ...
       <div class="form-content">
        <input class="form-input" placeholder="请输入持仓间隔" type="text"/>
       </div>
      </div>
      <div class="form-row items-center">
       <label class="form-label !pt-0">
   ...
   <div class="section-card">
    ...
    <div class="section-body">
     <div class="form-row items-center">
      ...
      <div class="form-content">
       <select v-model="form.netRewardEnabled" class="form-select">
        <option value="">
         请选择
        </option>
        <option value="1">
     ...
     <div id="netDepositRewardConfigSection">
      <div class="form-row items-center">
       <label class="form-label !pt-0">
        <span class="required-star">
         *
      ...
      <div class="form-row items-center">
       ...
       <div class="form-content flex items-center gap-6">
        <label class="flex items-center cursor-pointer">
         <input checked="" class="w-4 h-4 text-primary focus:ring-primary border-gray-300" name="start_time" type="radio"/>
         <span class="ml-2 text-sm text-gray-700">
          注册时间
         </span>
        </label>
        <label class="flex items-center cursor-pointer">
         <input class="w-4 h-4 text-primary focus:ring-primary border-gray-300" name="start_time" type="radio"/>
         <span class="ml-2 text-sm text-gray-700">
          规则启动时间
         </span>
        </label>
      ...
      <div class="form-row items-center">
       ...
       <div class="form-content flex gap-3">
        <select class="form-select !w-[160px]" id="tradeVolumeRuleSelect">
         <option value="">
          请选择
         </option>
         <option value="single">
        ...
        <input class="form-input !w-[228px]" id="tradeVolumeInput" placeholder="请输入交易量" type="text"/>
        <!-- 分组/代理计算：点击配置按钮 -->
      ...
      <div class="form-row items-center">
       ...
       <div class="form-content flex items-center gap-6">
        <label class="flex items-center cursor-pointer">
         <input v-model="form.netRewardType" class="w-4 h-4 text-primary focus:ring-primary border-gray-300" name="net_reward_type" type="radio" value="unified"/>
         <span class="ml-2 text-sm text-gray-700">
          统一计算
         </span>
        </label>
        <label class="flex items-center cursor-pointer">
         <input v-model="form.netRewardType" class="w-4 h-4 text-primary focus:ring-primary border-gray-300" name="net_reward_type" type="radio" value="rule"/>
         <span class="ml-2 text-sm text-gray-700">
          规则计算
         </span>
        </label>
      ...
      <div class="form-row items-center">
       ...
       <div class="form-content">
        ...
        <div class="flex gap-3" id="netRewardInputs">
         <input class="form-input !w-[194px]" placeholder="请输入净入金值" type="text"/>
         <input class="form-input !w-[194px]" placeholder="请输入净入金奖励比例" type="text"/>
        </div>
        <!-- 规则计算：点击配置按钮 -->
   ...
   <div class="flex justify-center gap-4 mt-8 pt-6 border-t border-gray-200">
    <button class="px-8 py-2 border border-gray-300 text-gray-700 bg-white rounded hover:bg-gray-50 transition-colors text-sm font-medium" type="button" @click="goBack">
     返回
    </button>
    <button class="px-8 py-2 bg-[#1D64D8] text-white rounded hover:bg-blue-700 transition-colors text-sm font-medium" type="button" @click="submit">
     提交
    </button>
   </div>
  </div>
...
const form = reactive({
  ...
  salesName: '',
  tradeRewardEnabled: '',
  tradeRewardType: 'unified',
  netRewardEnabled: '',
  netRewardType: 'unified'
})

watchEffect(() => {
  const id = String(route.query.id || '').trim()
  form.id = id || 'S-001'
  if (!form.salesName) form.salesName = form.id === 'S-002' ? '张三 (zhangsan@qq.com)' : '刘浩 (liuhao@qq.com)'
})
...
const goEdit = () => {
  router.push({ path: '/crm/sales-edit', query: { id: String(form.id || '') } })
}

const submit = () => {
  router.push({ path: '/crm/sales' })
}
</script>
